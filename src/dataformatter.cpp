...
{
/*
    const char *fileName = "abstractccinterface.cpp";
    std::ofstream ofs(fileName);
    */
    
    
    /*qtコマンドです
    QFile flFile("abstractccinterface.cpp");

    if (flFile.open(QIODevice::WriteOnly)) {
           QTextStream out(&flFile);
           out << "ファイルへ書き込むをする方法。" << endl;
           out << "まず、ファイルを開きます。" << endl;
           out << "テキストデータの場合は、TextStreamを使って書き込みます。" << endl;
           out << "endlをつけると改行します。" << endl;
    ...
    */
    ...
    for (uint32_t i = 0; i < input->height; i++)
    {
        for (uint32_t j = 0; j < input->width; j++)
        {
            ...
            //printf("%lf",val); 0ばっかり出力される

          //  printf("%lu　",val);
           // scanf("%d",&check);
            //if(check == 1){ofstream ofs("debug.log");
           // std::ofstream ofs("test4.txt");
            //cout.rdbuf(ofs.rdbuf());
            float boss;
            boss = (val - 27315) / 100.0;
        // ここからファイル出力を試みる
         ...
         //ここまで
           // std::cout << std::setprecision(4) << boss << " ";//ファイルに書いたら早くなる　０mq　バッファ　標準的な出力　osstream バイナリで出てくる可能性がある　ファイル名区別名.txt
            //ostringstream　（取得時間　x y データ）unixtime  ms!!　タイムスタンプとして使う　chrono?
            //}
...
/*
    ...
    m_minVal = 20; //-273で固定される　使えないことはないけど気になる
    m_maxVal = 40;
    minValChanged(minval);
    maxValChanged(maxval);

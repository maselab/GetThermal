...
void DataFormatter::FindMinMax(const uvc_frame_t *input, QPoint &minPoint, uint16_t &minVal, QPoint &maxPoint, uint16_t &maxVal) const
{
    ...
    const char *filename_prefix = "/tmp/gt_";
    const char *filename_postfix = ".csv";

    pid_t pid = getpid();
    char pid_str[32];
    sprintf(pid_str, "%s%d%s", filename_prefix, pid, filename_postfix);

    std::ofstream ofs(pid_str, std::ios::app);
    ...
    for (uint32_t i = 0; i < input->height; i++)
    {
        for (uint32_t j = 0; j < input->width; j++)
        {
            ...
            // OUT
            if(i == input->height - 1 && j == input->width - 1){
              ofs << boss << "\n";
            }else{
              ofs << boss << ",";
            }
